...
function ProjectCard({
  ...
  return (
    <a
      ...
      className="group flex flex-col gap-1.5 rounded-lg border-2 border-dotted border-border p-3 transition-colors hover:border-primary/40"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <h3 className="font-doto text-base leading-tight font-bold text-primary">
            {project.title}
          </h3>
      ...
      <div className="flex flex-wrap gap-1">
        {project.technologies.map((tech) => (
          <span
            key={tech}
            className="inline-flex items-center rounded-md border border-border px-1.5 py-0.5 font-mono text-[10px] text-primary/50"
          >
            {tech}
          </span>
        ))}
...
export default function ProjectsSection() {
  return (
    <section className="flex flex-col gap-2.5">
      <span className="font-mono text-[10px] tracking-wider text-primary/40 uppercase">
        Projects
      </span>
